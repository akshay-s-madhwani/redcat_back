    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if(connection === 'open') {
        ...
        resolve({
          ...
          async sendSimpleMessage( number: string, content: string): Promise<proto.WebMessageInfo | any > {
...
(async () => {
  ...
  redis.on('message',async (channel:string , message:any)=>{
    ...
    if(channel === 'check_number'){
    	let result = await client.sendSimpleMessage(message , 'verify_number')
    	console.log(result);
    	...
    	if(channel === 'registration_channel'){
    		...
    		await client.sendSimpleMessage(`${number}` , `Hello and Welcome to Redcat Merchant Management\nPlease enter this OTP into verification page \n\n*${otp}*`);
    		}
  })
